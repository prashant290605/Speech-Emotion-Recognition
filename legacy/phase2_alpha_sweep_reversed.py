...
def main() -> None:
    log_step("[phase2-reverse] Loading datasets")
    ...
    log_step("[phase2-reverse] Preparing wav2vec2 features")
    ...
    log_step("[phase2-reverse] Preparing MFCC features")
    ...
    log_step("[phase2-reverse] Applying CORAL")
    X_w2v_aligned = coral_align(X_w2v_train, X_w2v_test)
    rows: list[dict[str, float]] = []

    print("===== PHASE 2 RESULTS (ALPHA SWEEP) =====")
    ...
    for alpha in ALPHAS:
        log_step(f"[phase2-reverse] Training alpha={alpha:.2f}")
    ...
    save_result(
        "phase2_alpha_sweep_reversed.json",
        {
            "phase": "phase2_alpha_sweep_reversed",
            "train_dataset": "CREMA-D",
            "test_dataset": "RAVDESS",
