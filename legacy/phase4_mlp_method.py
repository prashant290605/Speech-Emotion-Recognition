...
def main() -> None:
    set_seed()
    log_step("[phase4] Loading datasets")
    ...
    log_step("[phase4] Preparing wav2vec2 features")
    ...
    log_step("[phase4] Preparing MFCC features")
    ...
    log_step("[phase4] Applying CORAL")
    X_w2v_aligned = coral_align(X_w2v_train, X_w2v_test)
    rows: list[dict[str, float]] = []

    print("===== PHASE 4 RESULTS (MLP + METHOD) =====")
    print()
    print("Alpha | Accuracy | F1 Score")
    print("--------------------------------")

    for alpha in ALPHAS:
        log_step(f"[phase4] Training alpha={alpha:.2f}")
