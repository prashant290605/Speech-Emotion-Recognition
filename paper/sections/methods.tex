...
$X_s \mid y=k$ and $X_t \mid y=k$. Its bandwidth is the seeded median
pairwise distance in that class's pooled source and target samples; the
normaliser is the mean absolute MMD$^2$ across five random half-splits of the
aligned source samples for that same class. A class is undefined rather than
estimated when either side has fewer than 50 examples. The reported conditional
value is the unweighted mean over defined classes, first within seed and then
over seeds. Thus its ratio to marginal MMD is a descriptive comparison of two
normalised diagnostics, not a probability decomposition or an estimate of
$P(y \mid x)$.
...
together, 2000 replicates \cite{cameron2008cluster}. Speakers are the resampling unit because
