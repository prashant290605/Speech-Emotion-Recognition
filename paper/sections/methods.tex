...
is also the index order of every confusion matrix reported in supplementary
Section~S6. Two mapping
...
that all corpus differences arise from the mapping. Prior divergence and
the label-harmonisation controls are reported in
Section~\ref{sec:results-robustness} and supplementary Sections~S1 and~S2.

\paragraph{Label-harmonisation robustness protocol.} Two controls were
pre-specified to test whether the aligned-versus-unaligned conclusion depends
on this mapping. Supplementary Section~S1 states their label spaces, ledgers
and run identifiers, and reports them.
...
warm-started from the CORAL solution (full) or a diagonal moment match
(diagonal), with a parameter-count-normalised learning rate.

\paragraph{Fallback, and why it must be reported.} An unpenalised MMD check on
the first 400 observations compares the candidate with its warm start, and the
returned map reverts to the warm start when the candidate worsens that check.
This differs from the penalised minibatch training objective, so rejection is
not proof of failure to improve that objective. Across the main grid the check
rejects the candidate in \textbf{64.9\%} of \texttt{mkmmd\_diag} runs and
\textbf{55.2\%} of \texttt{mkmmd\_full} runs; in those cells the fitted map is
the warm start, the diagonal moment match or CORAL respectively. Reporting
either as a distinct learned map without saying so would overstate what was
actually fitted. These rates qualify every MK-MMD comparison in this paper: the
experiment evaluates these affine maps, this warm start and this optimisation
budget, and does not establish what a differently optimised MK-MMD map would
achieve. Supplementary Section~S8 records the procedure and the learning-rate
derivation in full.
...
Five families are evaluated: logistic regression, a linear support vector
machine (SVM), an SVM with a radial basis function (RBF) kernel, a multilayer
perceptron (MLP), and a small Transformer head over the 8-segment sequence. Each is searched with
...
in supplementary Section~S3 excludes it. The translation audit
excludes it by construction and is unaffected.
