...
## 2026-08-25 — Phases 10 and 11: figures, LaTeX tables, RESULTS.md

All experiments complete. Nothing new was run; everything below regenerates
from the result files.

### Sweep completed and both verdicts re-checked at full coverage

The three sweep workers finished: **2340/2340, zero failures, all six rungs
390/390**. Regenerating the report at full coverage did not change either
verdict, only sharpened them:

* **Frame dependence** — pooled rho own **-0.444** [-0.505, -0.383] against
  reference **+0.142** [+0.062, +0.221]. Opposite signs, neither interval
  covering zero. 11 of 36 cells disagree by the strict per-cell test (was 7/24
  on four rungs).
* **Depth divergence** — pooled gap **-0.17 layers**, 2 of 36 cells positive.
  The retraction stands.

### Phase 10 — per-class analysis and figures

`tools/phase10_per_class.py` computes precision, recall, F1 and support from the
**stored predictions** rather than the per-class row columns, so all three come
from one confusion matrix and cannot disagree. Cluster bootstrap over speakers
and seeds, 2000 replicates.

`sad` and `happy` fall below macro-F1 in **both** directions; no class collapsed
to zero predictions anywhere. Weakest to strongest forward: neutral 0.255, sad
0.280, happy 0.329, fear 0.334, disgust 0.363, angry 0.440.

Seven figures, each regenerated by script from `results/` so none can drift from
the tables. Style rules live in `src/ser/figures.py` and are enforced rather
than remembered: vector PDF at journal column width, Type 42 fonts (matplotlib
defaults to Type 3, which several publishers reject), Okabe-Ito palette, and
**colour never carrying meaning alone** -- `series()` returns colour *and*
marker *and* linestyle together so a call site cannot take only the colour.

The confusion figure fixes every defect in the original Figure 2: class labels
on both axes, both axes named, one shared colourbar, row-normalised so it shows
confusion rather than class priors, value printed in every cell, panels large
enough to read.

Two figure claims are now **derived from the data rather than typed**: the
per-class title names whichever classes are below macro in both directions, and
the depth "cost" column is reported against a **random-layer null** -- without
it the cost is non-negative by construction and proves nothing. Against that
null, `source_val` selection is *better* by -0.0366 [-0.0448, -0.0285], which is
what contradicts the "systematically picks the wrong depth" framing.

### Phase 11 — LaTeX tables and release

`src/ser/latex.py` emits booktabs tables and nothing more exotic: no `siunitx`,
no `multirow`, no `tabularx`. A table that fails to compile in the venue's
template gets retyped by hand, which is what this module exists to prevent.
Seven tables in `tables/`, each carrying its own run filter as a note so a
reader can see which slice of 5424 rows produced it without leaving the page.

Two bugs caught while building them, both of the class this project keeps
finding: the header row was escaped even when the caller passed
`escape_cells=False`, turning `$ho$` into visible backslashes; and the "sign
differs" column used a weaker test than the report, so the table would have
disagreed with `layer_sweep_v2.md`. Both are now covered by tests.

README rewritten for release: headline numbers with intervals and floors, a
clean-clone reproduction path, the frozen config tags, and explicit sections on
the **leakage assertions** and the **A10 firewall** describing what is enforced
mechanically rather than by convention.

### reports/RESULTS.md

Eleven sections, every number with its interval, its floor and its run filter,
generated by `tools/make_results_doc.py`. Completeness over selectivity, as
instructed: it contains the **retractions** (seven claims withdrawn, narrowed or
falsified during this rebuild) and the **null results** (aligned rungs barely
differ; more moments do not help; label-shift correction hurts; lambda is not
worth searching; depth divergence does not generalise; zero failures in 8004
runs) alongside the confirmations.

### Files

Created: `src/ser/figures.py`, `src/ser/latex.py`, `tools/phase10_per_class.py`,
`tools/make_figures.py`, `tools/make_tables.py`, `tools/make_results_doc.py`,
`tests/test_figures_and_latex.py`, `reports/RESULTS.md`, `reports/per_class.md`,
`reports/per_class.json`, 7 figures in `figures/`, 7 tables in `tables/`.

Modified: `README.md` (release), `reports/layer_sweep_v2.md` (full coverage).

441 tests pass.

### Open

* Five reference DOIs still need manual resolution.
* IEMOCAP was never acquired -- the licence needs a faculty signatory. Every
  claim is over two corpora, and the manuscript says so.
* The manuscript itself is not written. `reports/RESULTS.md` is what it gets
  written from.
