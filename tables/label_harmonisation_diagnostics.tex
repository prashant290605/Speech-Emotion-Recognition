\begin{table*}[!t]
  ...
  \begin{tabular}{lllrr}
    ...
    calm dropped six & RAVDESS $\rightarrow$ CREMA-D & \texttt{mean\_shift} & 0.0601 & 0.1736 \\
    ...
    calm dropped six & CREMA-D $\rightarrow$ RAVDESS & \texttt{mean\_shift} & 0.0862 & 0.1888 \\
    ...
    neutral excluded five & RAVDESS $\rightarrow$ CREMA-D & \texttt{mean\_shift} & 0.0646 & 0.1573 \\
    ...
    neutral excluded five & CREMA-D $\rightarrow$ RAVDESS & \texttt{mean\_shift} & 0.0960 & 0.1899 \\
    neutral excluded five & CREMA-D $\rightarrow$ RAVDESS & \texttt{coral} & 0.0916 & 0.1494 \\
    \bottomrule
  \end{tabular}
  \\[2pt]
  \begin{minipage}{\linewidth}\footnotesize Derived from the two mapping-specific diagnostic ledgers. Each row averages five seeds. Controls: HuBERT final-layer features, logistic regression, and \texttt{none}, \texttt{zscore}, \texttt{mean\_shift}, and \texttt{coral} ($\varepsilon=10.0$). Conditional values are an unweighted mean over the retained classes and read target labels only after the alignment leakage assertion. No conditional-to-marginal ratio is reported.\end{minipage}
\end{table*}
